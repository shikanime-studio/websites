...
const makers = sqliteTable("makers", {
  ...
  links: text("links", { mode: "json" }).notNull().$type().default(sql`'[]'`),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" }).notNull()
});
const usersToMakers = sqliteTable(
  "users_to_makers",
  {
    userId: text("user_id").notNull().references(() => users.id),
...
const usersToMakersRelations = relations(usersToMakers, ({ one }) => ({
  user: one(users, {
...
const usersToLicenses = sqliteTable(
  "users_to_licenses",
  {
    userId: text("user_id").notNull().references(() => users.id),
...
const usersToLicensesRelations = relations(
  usersToLicenses,
  ({ one }) => ({
    user: one(users, {
...
const usersToCharacters = sqliteTable(
  "users_to_characters",
  {
    userId: text("user_id").notNull().references(() => users.id),
...
const usersToCharactersRelations = relations(
  usersToCharacters,
  ({ one }) => ({
    user: one(users, {
...
const eventsToMakers = sqliteTable(
  "events_to_makers",
  {
...
const eventsToMakersRelations = relations(eventsToMakers, ({ one }) => ({
  event: one(events, {
...
const licenseRelations = relations(licenses, ({ many }) => ({
...
const items = sqliteTable("items", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  imageUrls: text("image_urls", { mode: "json" }).notNull().$type().default(sql`'[]'`),
...
const itemsToLicenses = sqliteTable(
  "items_to_licenses",
  {
...
const itemsToLicensesRelations = relations(
  itemsToLicenses,
  ({ one }) => ({
    item: one(items, {
...
const makersRelations = relations(makers, ({ many }) => ({
  items: many(items),
...
const charactersRelations = relations(characters, ({ many }) => ({
...
const schema = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  eventsRelations,
  eventsToMakers,
  eventsToMakersRelations,
  items,
  itemsRelations,
  itemsToLicenses,
  itemsToLicensesRelations,
  licenseRelations,
  licenses,
  makers,
  makersRelations,
  ...
  users,
  usersToCharacters,
  usersToCharactersRelations,
  usersToLicenses,
  usersToLicensesRelations,
  usersToMakers,
  usersToMakersRelations,
  verifications,
  votes,
  votesRelations
