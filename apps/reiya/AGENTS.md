...
- The UI copy/tagline in the base layout is: “Let’s bankrupt together” (see
  \[BaseLayout.astro\](src/layouts/BaseLayout.astro)).
- Logged-in users get additional navigation (e.g., Following) (see
  \[Navbar.astro\](src/components/Navbar.astro)).

## URLs

- Production: `https://reiya.shikanime.studio` (see
...
- Server secrets (Cloudflare): `BETTER_AUTH_SECRET`, `GOOGLE_CLIENT_SECRET` (see
...
- Auth: Google One Tap is triggered client-side (see
  \[BaseLayout.astro\](src/layouts/BaseLayout.astro)).
- Persistence: D1 database binding `DB` (see
