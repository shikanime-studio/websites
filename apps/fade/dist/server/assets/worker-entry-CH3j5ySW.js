  ...
  */
  constructor(options, getStoreConfig) {
    ...
    this.clearCache = (opts) => {
      ...
      if (filter !== void 0) this.stores.setCachedMatches(this.stores.cachedMatchesSnapshot.state.filter((m2) => !filter(m2)));
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./_tanstack-start-manifest_v-MUrYpRhN.js");
...
async function loadEntries() {
  const routerEntry = await import("./router-BRIDf2Ri.js").then((n2) => n2.w);
  return {
