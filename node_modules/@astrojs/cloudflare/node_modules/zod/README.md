...
1. Zod uses `.then` to attach an additional validation step onto the existing Promise. You'll have to use `.catch` on the returned Promise to handle validation failures.
...
1. The second argument accepts some options. You can use this to customize certain error-handling behavior:
...
1. Otherwise, the data is parsed using the base schema
...
1. If the parsing fails, the "catch value" is returned
