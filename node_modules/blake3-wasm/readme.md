...
1. Run `make prepare-binaries`. This will update the branch `generate-binary`, which kicks off a build via Github actions to create `.node` binaries for every relevant Node.js version.
1. When the build completes, it'll generate a zip file of artifacts. Download those.
1. Back on master, run `npm version <type>` to update the version in git. `git push --tags`.
1. On Github, upload the contents of the artifacts folder to the release for the newly tagged version.
1. Run `npm publish`.
