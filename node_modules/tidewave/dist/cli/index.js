...
Defaults to 30000 (30 seconds).`),
  ...
  docsInputSchema = exports_external.object({
    reference: exports_external.string().describe(referenceDescription)
  });
  sourceInputSchema = exports_external.object({
    reference: exports_external.string().describe(referenceDescription)
  });
...
`);
  ...
  if (symbol.valueDeclaration || symbol.declarations && symbol.declarations.length > 0) {
    ...
    if (ts3.isFunctionDeclaration(declaration) && declaration.name) {
      const functionType2 = checker.getTypeOfSymbolAtLocation(symbol, declaration);
      const functionSignatures = checker.getSignaturesOfType(functionType2, ts3.SignatureKind.Call);
...
var exports_code_executor = {};
__export(exports_code_executor, {
...
var init_src = __esm(() => {
  ...
  Tidewave = {
    ...
    getSourceLocation,
    formatOutput,
    async executeIsolated(request) {
      const { executeIsolated: executeIsolated2 } = await Promise.resolve().then(() => (init_code_executor(), exports_code_executor));
      return executeIsolated2(request);
    }
  };
});
...
var require_ajv = __commonJS((exports, module) => {
  ...
  function addFormat(name2, format) {
    if (typeof format == "string")
      format = new RegExp(format);
    this._formats[name2] = format;
    return this;
  }
  ...
  function addInitialFormats(self) {
    for (var name2 in self._opts.formats) {
      var format = self._opts.formats[name2];
      self.addFormat(name2, format);
    }
  }
  function addInitialKeywords(self) {
    for (var name2 in self._opts.keywords) {
      var keyword = self._opts.keywords[name2];
      self.addKeyword(name2, keyword);
    }
  }
...
class McpServer {
  ...
  _createRegisteredTool(name2, title, description, inputSchema, outputSchema, annotations, callback) {
  ...
  tool(name2, ...rest) {
    ...
    return this._createRegisteredTool(name2, undefined, description, inputSchema, outputSchema, annotations, callback);
  }
  registerTool(name2, config, cb) {
    ...
    return this._createRegisteredTool(name2, title, description, inputSchema, outputSchema, annotations, cb);
  }
  prompt(name2, ...rest) {
...
async function handleGetLogs(args) {
  try {
    ...
    const output = logs.map((log) => `[${log.timestamp}] ${log.severityText}: ${log.body}`).join(`
`);
    return {
      content: [
        {
...
async function serveMcp(transport) {
  ...
  server.registerTool(evalMcp.name, { description: evalMcp.description, inputSchema: evalMcp.inputSchema.shape }, handleProjectEvaluation);
  if (globalThis.__TIDEWAVE_CONSOLE_PATCHED__) {
    server.registerTool(logsMcp.name, {
...
var docsMcp, sourceMcp, evalMcp, logsMcp;
var init_mcp2 = __esm(() => {
  init_mcp();
  init_tools();
  init_package();
...
async function handleGetDocs2(modulePath, options) {
  if (options.prefix)
...
async function handleGetSourcePath2(moduleName, options) {
  if (options.prefix)
    chdir(options.prefix);
