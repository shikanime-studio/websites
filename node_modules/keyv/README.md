...
1. How to get started with running commands
1. How to contribute changes using Pull Requests
...
1. Clone the forked repository to your local directory using `git clone`
1. Install any of the above missing dependencies
...
1. From the root directory of your project, run the `yarn` command in the command prompt to install yarn.
1. Run the `yarn bootstrap` command to install any necessary dependencies.
1. Run `yarn test:services:start` to start up this project's Docker container. The container will launch all services within your workspace.
...
1. After making these changes, ensure that the project's tests still pass by executing the `yarn test` command in the root directory.
1. Commit your changes and push them to your forked repository.
1. Navigate to the original `keyv` repository and go the *Pull Requests* tab.
1. Click the *New pull request* button, and open a pull request for the branch in your repository that contains your changes.
1. Once your pull request is created, ensure that all checks have passed and that your branch has no conflicts with the base branch. If there are any issues, resolve these changes in your local repository, and then commit and push them to git.
1. Similarly, respond to any reviewer comments or requests for changes by making edits to your local repository and pushing them to Git.
1. Once the pull request has been reviewed, those with write access to the branch will be able to merge your changes into the `keyv` repository.
