...
1. Turn script tags into `noscript` tags to avoid scripts being executed.
1. Try to inline stylesheets to make sure local stylesheets can be used.
1. Make relative paths in href, src, CSS to be absolute paths.
1. Give an id to each Node, and return the id node map when snapshot finished.
...
1. Create some extra DOM node like text node to place inline CSS and some states.
1. Add data-extra-child-index attribute if Node has some extra child DOM.
